...
func main() {
	...
	capStore := subscribers.NewRedisCapStore(redisClient)
	subscriberService := subscribers.NewServiceWithConfig(subscriberRepo, inventoryRepo, subscribers.Config{
		...
		Creatives:    inventoryRepo,
		Caps:         capStore,
		Exposures:    subscriberRepo,
		Decisions:    subscriberRepo,
	})

	server := &http.Server{
		Addr: cfg.Addr,
		Handler: httpserver.NewPayloadRouter(httpserver.PayloadOptions{
	...
	go func() {
		log.Info("payload-api listening", slog.String("addr", cfg.Addr))
	...
	case <-ctx.Done():
		...
		if err := server.Shutdown(shutdownCtx); err != nil {
			log.Error("payload-api shutdown failed", slog.String("error", err.Error()))
			os.Exit(1)
		}
	case err := <-errCh:
		if err != nil && !errors.Is(err, http.ErrServerClosed) {
			log.Error("payload-api failed", slog.String("error", err.Error()))
			os.Exit(1)
		}
	}
}
